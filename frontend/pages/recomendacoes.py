...
def recomendar():
    ...
    username = st.selectbox("Seu ID de usuário", options=users_id)
    ...
    if st.button("Recomendar"):
        ...
        if response.status_code == 200:
            ...
            st.metric("Acurácia em relação ao teste", f"{resultado['acuracia_teste']*100:.0f}%")
            st.write(f"Acertos: {resultado['acertos']} de {resultado['total_teste']} livros interessantes ao leitor")
            ...
            st.divider()
            st.subheader("Livros de teste")

            columns = st.columns(3)

            for book_id in resultado["test_books"]:
                col = columns[j % 3]
                response = requests.get(f"{API_URL}/livro/capa_id/{book_id}")
                if response.status_code == 200:
                    livro = response.json()
